...
public class Gravity : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        foreach (GameObject stellarObject in GameObject.FindGameObjectsWithTag("Planet"))
        {
            if (stellarObject != gameObject)
            {
                //Debug.Log(stellarObject.name);
                if (Vector3.Distance(transform.position, stellarObject.transform.position) < 1 && gameObject.tag == "Star")
                {
